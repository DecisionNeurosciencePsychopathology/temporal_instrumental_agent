function [maxReward, constr, quits,cumReward] = ClockWalking_3D_plotter(options)
...
%3/2/2015 - Some changes made to the code: the function's input arg is
%clock_options.mat since if you want to run 'ClockWalker_fit_model' the simps
%function has an option argument that needs to be passed to this function. 
%There are also new plots made in this funciton compared to the
%older version we were using. 
...
elseif ~isstruct(options) %This is only for running simps
    ...
    episodeCount = 1000; %Split 500 IEV 500 DEV or run 1000 straight
    ...
    for jj=1:length(agents)
        ...
        switch agent
            case 'qlearning'
                if strcmp(cond,'DEV')
                    gamma = s_DEV.gamma;
                    alpha = s_DEV.alpha;
                    epsilon = s_DEV.epsilon;
                elseif strcmp(cond,'IEV')
                    gamma = s_IEV.gamma;
                    alpha = s_IEV.alpha;
                    epsilon = s_IEV.epsilon;
                else
                    gamma = s_QUADUP.gamma;
                    alpha = s_QUADUP.alpha;
                    epsilon = s_QUADUP.epsilon;
                end
            case 'sarsa'
                if strcmp(cond,'DEV')
                    gamma = s_DEV_sarsa.gamma;
                    alpha = s_DEV_sarsa.alpha;
                    epsilon = s_DEV_sarsa.epsilon;
                elseif strcmp(cond,'IEV')
                    gamma = s_IEV_sarsa.gamma;
                    alpha = s_IEV_sarsa.alpha;
                    epsilon = s_IEV_sarsa.epsilon;
                else
                    gamma = s_QUADUP_sarsa.gamma;
                    alpha = s_QUADUP_sarsa.alpha;
                    epsilon = s_QUADUP_sarsa.epsilon;
                end
        end
        ...
        for ei = 1:episodeCount,
            ...
            if diagnos == 1
                ...
                if strcmp(agent,'qlearning')
                    subplot(4,2,1+foo)
                    plot(1:length(quits),quits*10,'Linewidth',1.75) %I think this is it
                    axis([0 episodeCount 0 ntimesteps*10])
                    ylabel('Time x 10ms')
                    xlabel('Trials')
                    title('RT by Trial','FontWeight','bold')
                    
                    subplot(4,2,3+foo);
                    %plotyy(1:50,smooth(Q(1:ntimesteps, 1,1)),1:50,smooth(Q(1:ntimesteps,1,2)))
                    %hold on
                    plot((1:ntimesteps)*10,smooth(Q(1:ntimesteps, 1,1)),'Linewidth',1.75)
                    hold on
                    plot((1:ntimesteps)*10,smooth(Q(1:ntimesteps,1,2)),'r','Linewidth',1.75)
                    hold on
                    scatter(quits(1:ei)*10,quit_rews(1:ei),30,'k','filled')
                    hold off
                    ylabel('Reward')
                    xlabel('Time x 10ms')
                    title('Q_w_a_i_t(blue) Q_q_u_i_t(red) Rewards Sampled(circles)','FontWeight','bold')
                    if foo>1
                        hh=sprintf('Q-learning: Alpha %f, Gamma %f, Epsilon %f',alpha,...
                            gamma, epsilon);
                        xlabel({'Time x 10ms';hh})
                    end
                else
                    subplot(4,2,2+foo)
                    ...
                    title('Q_w_a_i_t(blue) Q_q_u_i_t(red) Rewards Sampled(circles)','FontWeight','bold')
                    if foo>1
                        hh=sprintf('SARSA: Alpha %f, Gamma %f, Epsilon %f',alpha,...
                            gamma, epsilon);
                        xlabel({'Time x 10ms';hh})
                    end
                end
