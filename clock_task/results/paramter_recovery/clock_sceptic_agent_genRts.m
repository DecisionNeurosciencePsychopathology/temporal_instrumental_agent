function [cost,v_it,rts,mov,ret] = clock_sceptic_agent_genRts(params, agent, rngseeds, cond, ntrials, nbasis, ntimesteps, reversal)
...
elseif strcmpi(agent, 'fixed_decay')
    ...
    alpha = params(3);
    gamma = params(4);
elseif strcmpi(agent, 'fixed_decay_autocorrelation')
    beta = params(2); %Should be fixed
    alpha = params(3);
    gamma = params(4);
end
    ...
    if strcmpi(agent, 'kalman_uv_logistic')
        ...
        if ismember(agent, {'fixedLR_egreedy', 'fixedLR_egreedy_grw'})
            ...
            if strcmp(autocorrelation,'exponential') && i>=2
                p_choice(i,:) = p_choice(i,:) + chi.*(lambda.^(abs((1:ntimesteps) - rts(i-1)))); %Use prev RT
...
%Cost function
%cost = -sum(ev_i);
cost = -sum((rts-p_chosen).^2); %maximize choice probabilities
