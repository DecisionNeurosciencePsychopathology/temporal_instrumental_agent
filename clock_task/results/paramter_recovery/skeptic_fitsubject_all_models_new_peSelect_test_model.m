...
%%
function [cost,ret,mov] = skeptic_fitsubject_all_models_new_peSelect_test_model(params, rt_obs, rew_obs, rngseeds, nbasis, ntimesteps, trial_plots, minrt, maxrt, modelname, fig_idx)
...
elseif strcmpi(modelname, 'v_fixed_rho_end_KL')
    prop_spread = params(1);
    beta = params(2);
    alpha = params(3); %learning rate
    rho = params(4);
...
elseif strcmpi(modelname, 'v_fixed_end_KL')
    prop_spread = params(1);
    beta = params(2);
    alpha = params(3); %learning rate
    kappa = params(4);
...
elseif strcmpi(modelname, 'v_process_noise_discounted')
    prop_spread = params(1);
    beta = params(2);
    omega = params(3); %scaling of process noise by PE.
    kappa = params(4);
...
%While creating rts these manipulate the choice rule
pch_exploit=0;
pch_noise=1;
...
rt_obs(1) = ceil(.5*ntimesteps); %%COPY THIS
cond = 'IEV';
...
sigma_noise = repmat(std(arrayfun(@(x) RewFunction(x*10, cond, 1), tvec))^2, 1, nbasis);
%[rew_obs ev_i] = RewFunction(rt_obs(1).*10, cond); %multiply by 10 because underlying functions range 0-5000ms %%COPY THIS

%I had to reset the rew_rng_seed to get different rts to generate.
rew_rng_seed = rngseeds(2);
rng(rew_rng_seed);
rew_rng_state=rng;
...
for i = 1:ntrials
    ...
    [rew_obs(i) ev_i(i)] = RewFunction(rt_obs(i).*10, cond); %multiply by 10 because underlying functions range 0-5000ms %%COPY THIS
    
    
    %1) compute prediction error, scaled by eligibility trace
    delta_ij(i,:) = e_ij(i,:).*(rew_obs(i) - mu_ij(i,:));
    
    delta_rt(i) = round(rt_obs(i)*nbasis/ntimesteps);
    if delta_rt(i)==0
        delta_rt(i)=1;
    end
    
    
    %Stash unupdated PE of mu
    mu_b4_PE(i,:) = mu_ij(i,:);
    
    
    
    if strcmpi(modelname, 'fixed') || strcmpi(modelname, 'fixed_KL') || strcmpi(modelname, 'v_fixed_end_KL')
        mu_ij(i+1,:) = mu_ij(i,:) + alpha.*delta_ij(i,:);
    elseif strcmpi(modelname, 'fixed_rho') || strcmpi(modelname, 'v_fixed_rho_end_KL')
        if delta_ij(i,delta_rt(i))>0
            mu_ij(i+1,:) = mu_ij(i,:) + alpha.*delta_ij(i,:);
        else
            mu_ij(i+1,:) = mu_ij(i,:) + rho.*delta_ij(i,:);
        end
    else
        ...
        %JW 10-12-15 So bsically for the v_process_noise model if the
        %initial reward is 0, sigma noise is 0
    ...
    %update reward expectation via discount if applicable (enduring version)
    if strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
        mu_ij_undisc(i,:) = mu_ij(i+1,:);
        vec = 1:nbasis;
        if delta_ij(i,delta_rt(i))>0
            discount = kappa*delta_ij(i,delta_rt(i))*vec; %% also add valence-dependent parameters: kappa for PE+, lambda for PE-
        else
            discount = lambda*delta_ij(i,delta_rt(i))*vec;
        end
        %update mu with discount after intial update
        mu_ij(i+1,:) = mu_ij(i+1,:) + discount;
        
        %Add any extra rt_expolit_dicounting code for plotting here!
        %
        %
        %
        discount_ij(i,:) = discount;
        
        v_jt_b4_PE = mu_b4_PE(i,:)'*ones(1,ntimesteps) .* gaussmat;
        v_func_b4_PE = sum(v_jt_b4_PE);
        v_it_b4_PE(i,:) = v_func_b4_PE; %Value curve per trial b4 PE update
        
        v_jt_undisc=mu_ij_undisc(i,:)'*ones(1,ntimesteps) .* gaussmat; 
        v_func_undisc = sum(v_jt_undisc);
        v_it_undis(i+1,:) = v_func_undisc; %Value curve per trial after PE update b4 discount
    end
    
    %Bound updated mu to 0
    mu_ij(i+1,mu_ij(i+1,:)<0) = 0;
    ...
    v_it(i+1,:) = v_func;
    v_it_current(i,:) = v_func;
    u_it(i+1,:) = u_func;
    
    
    %Value curves at t-1: Blue
    %Value curve after PE update Red
    %Value curve after PE and discount Red --
    %Super impose scatter of Rt's
    
    
%     if trial_plots==1
%         figure(66); clf;
%         plot(1:nbasis, mu_b4_PE(i,:), 'b')
%         hold on
%         if exist('mu_ij_undisc','var')
%             plot(1:nbasis, mu_ij_undisc(i,:), 'r')
%         end
%         plot(1:nbasis, mu_ij(i+1,:), 'g--')
%         plot(1:nbasis, delta_ij(i,:), 'k')
%         if isnan(unrew_rts(i))
%         scatter(rt_obs(i)*nbasis/ntimesteps,rew_obs(i),rew_obs(i), 'r','Filled');
%         else
%         scatter(rt_obs(i)*nbasis/ntimesteps,rew_obs(i),'b', 'Filled');
%         end
%         %indexmax = find(max(delta_ij(i,:)) == delta_ij(i,:));
%         str_mean = ['PE = ', num2str(delta_ij(i,delta_rt(i)))];
%         text(0, 0,str_mean)
% %         itIsRew = rew_obs>0;
% %         scatter(rt_obs(i),, 'r','Filled');
% %         scatter(rt_obs(i),unrew_rts(i),'b', 'Filled');
%         
%         
% %         if (0)
% %             figure(87); clf;
% %             plot(1:ntimesteps,v_it_b4_PE(i,:),'b')
% %             hold on
% %             plot(1:ntimesteps,v_it_undis(i+1,:),'r')
% %             hold on
% %             plot(1:ntimesteps,v_it(i+1,:),'g--')
% %             scatter(rt_obs(i)*24/500,rew_obs(i)+10, 'r','Filled');
% %             scatter(rt_obs(i)*24/500,unrew_rts(i),'b', 'Filled');
...
%     end
    ...
    if strcmpi(modelname, 'value_softmax') || strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'fixed') || strcmpi(modelname, 'fixed_rho') || strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
    ...
    elseif strcmpi(modelname, 'uv')
        ...
        v_final = uv_func;
        uv_it(i+1,:) = uv_func;
    elseif strcmpi(modelname, 'v_discounted') || strcmpi(modelname, 'fixed_KL')  || strcmpi(modelname, 'v_process_noise_discounted') || strcmpi(modelname, 'fixed_rho_KL') 
        vec = 1:ntimesteps;
        if delta_ij(i,delta_rt(i))>0
            discount = kappa*delta_ij(i,delta_rt(i))*vec; %% also add valence-dependent parameters: kappa for PE+, lambda for PE-
        else
        ...
        %         vec = 1:ntimesteps;
        %         discount = kappa*delta_ij(i,rt_obs(i))*vec; %% also add valence-dependent parameters: kappa for PE+, lambda for PE-
        v_final = v_func + discount;
        % re-write the discounted version of the value representation
        ...
        rt_obs(i+1) = rt_exploit_disc; %%COPY THIS maybe
    elseif strcmpi(modelname, 'uv_discounted')
        vec = 1:ntimesteps;
        if delta_ij(i,rt_obs(i))>0
            discount = kappa*delta_ij(i,delta_rt(i))*vec; %% also add valence-dependent parameters: kappa for PE+, lambda for PE-
        else
            discount = lambda*delta_ij(i,delta_rt(i))*vec;
        end
        %discount = kappa*delta_ij(i,rt_obs(i))*vec;
        v_final = v_func + discount;
        v_it_undisc(i+1,:) = v_func;
        v_it(i+1,:) = v_final;
...
% % % % %     
% % % % %     [val,ind] = max(p_choice(i,:)); %Choose max probability (exploit)
% % % % %     
% % % % %     %This will return 1 for a time point if we don't add an if
% % % % %     %statement,since the softmax returns very equal values
% % % % %     %rt_obs(i+1) = ind;
% % % % %     
...
if pch_exploit
    %% Find the weighted mean of the top 2 in p_choice
    
    %What if p_choice is completely equal??
    %if sum(unique(p_choice(i,:))==p_choice(i,:))>1
    if range(p_choice(i,:)) == 0
        ind = 250;
    else
        
        sorted_p=sort(p_choice(i,:));
        %Grab p_choice values at max
        top_pch = sorted_p(end-1:end);
        
        %For some reason the range is different but MATLAB still returns two
        %top numbers that are the same sometimes
        if range(top_pch) == 0
            ind = 250;
        else
            %grab rts at max p_choice
            rt(1)=find(p_choice(i,:)==top_pch(1));
            rt(2)=find(p_choice(i,:)==top_pch(2));
            ind = (rt(1)*top_pch(1)+rt(2)*top_pch(2))./(sum(top_pch));
        end
        %w= %weighted vector
        %mean(w.*top_max,2)
        %ind = mean(top_pch);
...
%cost = -sum(log(p_chosen)); %maximize choice probabilities
%fprintf('The cost for this subjects run: %.2f\n', cost)

%New cost diff os sum of squared rts
%cost = -sum((rt_obs(1:50)'-rts_pred_exploit(1:50)).^2); %maximize choice probabilities
cost = -sum((rt_obs(2:end)'-p_chosen).^2); %maximize choice probabilities

%Rmsearch was breaking down added this to keep it from going to inf
if cost==inf
    cost = 9999999999;
end
...
if strcmpi(modelname, 'v_processnoise')
    ret.sigma_nosie = sigma_noise;
end
...
if trial_plots
    ...
    %         hold off;
    %
    %% Some trialwise diagnostics of variables affected by the parameters
%     figure(fig_idx*100); clf;
%     %Expected reward
%     subplot(3,1,1)
%     contourf(1:ntrials,1:nbasis,mu_ij(1:ntrials))
%     title('mu ij')
%     %PE
%     subplot(3,1,2)
%     contourf(1:ntrials,1:nbasis,delta_ij(1:ntrials))
%     title('delta ij')
%     %Discount
%     subplot(3,1,3)
%     contourf(1:ntrials,1:nbasis,discount_ij(1:ntrials))
%     title('discount ij')
    
    
    
    rt_obs = rt_obs(1:ntrials);
    
    %% find unrewarded RTs
    unrew_rts = NaN(size(rt_obs));
    unrew_rts(rew_obs==0) = rt_obs(rew_obs==0);
    
    if strcmpi(modelname, 'value_softmax') || strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'fixed') || strcmpi(modelname, 'fixed_KL') || strcmpi(modelname, 'v_process_noise_discounted') || strcmpi(modelname, 'fixed_rho')...
            || strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
        subplot(3,1,1);
        hold on;
        scatter(1:ntrials, rt_obs(1:ntrials),rew_obs(1:ntrials)+10, 'r','Filled');
        scatter(1:ntrials, unrew_rts,'b', 'Filled');
        if strcmpi(modelname, 'value_softmax') || strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'fixed') || strcmpi(modelname, 'v_process_noise_discounted') || strcmpi(modelname, 'fixed_rho') || strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
            plot(1:ntrials, rts_pred_exploit(1:ntrials), 'b', 'LineWidth', 3);
        elseif strcmpi(modelname, 'fixed_KL')
            scatter(1:ntrials, rt_obs(1:ntrials),rew_obs(1:ntrials)+10, 'r','Filled');
            scatter(1:ntrials, unrew_rts(1:ntrials),'b', 'Filled');
            plot(1:ntrials, rts_pred_exploit_disc(1:ntrials), 'b', 'LineWidth',3);
        end
        title(modelname)
        hold off;
        subplot(3,1,2);
        ...
        colormap(summer);
        ax3 = subplot(3,1,3);
        if strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'v_process_noise_discounted')
            contourf(1:ntrials, 1:nbasis, ret.Q_ij(1:ntrials,:)'); hold on;
            title('Process noise map');
            colormap(ax3,summer);
        else
            ax3 = subplot(3,1,3);
            contourf(1:ntrials, 1:ntimesteps, ret.p_choice(1:ntrials,:)'); hold on;
            scatter(1:ntrials, rt_obs,rew_obs+10, 'r', 'Filled');
            scatter(1:ntrials, unrew_rts,'b', 'Filled'); hold off;
            title('Discounted choice probability map; red: rewards,   blue: ommissions');
        end
...
%        k = waitforbuttonpress;
    ...
    elseif strcmpi(modelname, 'v_discounted')
        ...
        scatter(1:ntrials, rt_obs,rew_obs+10, 'r','Filled');
        scatter(1:ntrials, unrew_rts,'b', 'Filled');
        plot(1:length(rts_pred_exploit), rts_pred_exploit, 'b', 'LineWidth', 3);
        plot(1:length(rts_pred_exploit_disc), rts_pred_exploit_disc, 'g*-', 'LineWidth', 3);
        hold off;
