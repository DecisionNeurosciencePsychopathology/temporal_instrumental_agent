function [maxReward, constr, quits,cumReward,mov, ret] = ClockWalking_3D_discountedEv_forRMsearch(options,rt_obs,~,m,rngseeds,params,plot_index,gra_options)
...
if(nargin < 1),
    ...
    gamma = params(1); %discount factor: how important are are short-term versus long-term gains (0 = short-sighted, 1 = long-term payoff)
    alpha = params(2); %learning rate: how much to weight new information (0 = no learning, 1 = only value latest information)
    epsilon = params(3); %how often does agent explore? 10% here
    lambda = params(4);
    ntimesteps = options.ntimesteps; %how many timesteps for the trial
...
%             end
            ...
            while(episodeFinished == 0 && curpos.row < ntimesteps)
                ...
                if a == 2 %i.e., quit
                    ...
                    %Had to change this since we weren't using the matrix,
                    %or should we??
...
%                     [~,ev_i(ei)] = RewFunction(nextpos.row.*factor, cond);
        ...
        %First count number of nans
        gen_nans=find(isnan(quits));
        org_nans=find(isnan(rt_obs));
        
        %Weird way of combining matrices of different lengths
        [i1,j1] = ndgrid(1:size(gen_nans,1),1:size(gen_nans,2));
        [i2,j2] = ndgrid(1:size(org_nans,1),(1:size(org_nans,2))+size(gen_nans,2));
        z = accumarray([i1(:),j1(:);i2(:),j2(:)],[gen_nans(:);org_nans(:)]);
        
        %Find all the nans
        all_nans = zeros(1,episodeCount);
        all_nans(unique(z(z~=0)))=1;
        
        %Compute penalty
        same_nan_choice=sum(ismember(org_nans, gen_nans));
        if numel(gen_nans)>numel(org_nans)
            penalty=numel(gen_nans)-same_nan_choice;
        else
            penalty=numel(gen_nans)-same_nan_choice;
        end
        
        %maxReward = -sum(cumReward);
        maxReward = sum((rt_obs(~all_nans)-quits(~all_nans)' + (25*penalty)).^2);
        %update plot index
