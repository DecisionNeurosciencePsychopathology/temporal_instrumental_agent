...
%%
function [cost,ret,mov] = skeptic_fitsubject_all_models_new_peSelect(params, rt_obs, rew_obs, rngseeds, nbasis, ntimesteps, trial_plots, minrt, maxrt, modelname, fig_idx,sigma_noise)
...
%this is just manually set for now...!!!!
%cond='IEV';
%sigma_noise = repmat(std(arrayfun(@(x) RewFunction(x*10, cond, 1), tvec))^2, 1, nbasis); %will this screw up randsample??
...
if size(rt_obs(1:50),2)~=size(p_chosen,2)
    rt_obs = rt_obs';
end
...
%     title('discount ij')
    ...
    if strcmpi(modelname, 'value_softmax') || strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'fixed') || strcmpi(modelname, 'fixed_KL') || strcmpi(modelname, 'v_process_noise_discounted') || strcmpi(modelname, 'fixed_rho')...
        ...
        if strcmpi(modelname, 'value_softmax') || strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'fixed') || strcmpi(modelname, 'v_process_noise_discounted') || strcmpi(modelname, 'fixed_rho') || strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
            ...
            plot(1:ntrials, p_chosen(1:ntrials), 'b', 'LineWidth', 3);
        elseif strcmpi(modelname, 'fixed_KL')
            scatter(1:ntrials, rt_obs,rew_obs+10, 'r','Filled');
            scatter(1:ntrials, unrew_rts,'b', 'Filled');
        ...
        colormap(summer);
        ax3 = subplot(3,1,3);
        if (strcmpi(modelname, 'v_processnoise') || strcmpi(modelname, 'v_process_noise_discounted') & any(sum(ret.Q_ij(1:ntrials,:)',2))~=0)
