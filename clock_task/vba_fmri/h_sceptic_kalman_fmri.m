...
% AD: not sure we need this for fMRI
H = calc_shannon_H(( w / sum( w ) )); %Entropy
...
if inF.kalman.kalman_uv_sum
    ...
    w = w + k.*delta;
    if inF.u_aversion
        fx(hidden_state_index(:,1))= w + tau.*sigma; %mix together value and uncertainty according to tau
    else
        fx(hidden_state_index(:,1))=tau.*w + (1-tau).*sigma; %mix together value and uncertainty according to tau
    end
...
elseif inF.kalman.fixed_uv
    ...
    % compute k
    fx(hidden_state_index(:,1)) = w + k.*delta;
end
