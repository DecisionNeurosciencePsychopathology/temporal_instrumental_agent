...
if ~isfield(so, 'autocorrelation'), so.autocorrelation = 'none'; end
...
%Setting up additional argu for kalman models so the script doesn't break
if ~isfield(so, 'kalman'), so.kalman = []; end
if ~isfield(so.kalman, 'fixed_uv'), so.kalman.fixed_uv = 0; end
if ~isfield(so.kalman, 'kalman_uv_sum'), so.kalman.kalman_uv_sum = 0; end
if ~isfield(so.kalman, 'kalman_softmax'), so.kalman.kalman_softmax = 0; end
if ~isfield(so.kalman, 'kalman_logistic'), so.kalman.kalman_logistic = 0; end
...
elseif strcmpi(so.model,'fixed_UV')
    ...
    %You really do need additional para in here...
    so.kalman.fixed_uv = 1;
end
