...
%%
function [cost,ret,mov] = skeptic_fitsubject_all_models(params, rt_obs, rew_obs, rngseeds, nbasis, ntimesteps, trial_plots, minrt, maxrt, modelname, fig_idx)
...
for i = 1:ntrials
    ...
    if strcmpi(modelname, 'v_fixed_rho_end_KL') || strcmpi(modelname, 'v_fixed_end_KL')
        ...
        if (mean(delta_ij(i,:)))>0
            ...
            discount = lambda*(mean(delta_ij(i,:)))*vec;
        end
    ...
    elseif strcmpi(modelname, 'uv')
        ...
        v_final = uv_func;
        uv_it(i+1,:) = uv_func;
    elseif strcmpi(modelname, 'v_discounted') || strcmpi(modelname, 'fixed_KL')  || strcmpi(modelname, 'v_process_noise_discounted')
        vec = 1:ntimesteps;
        if (mean(delta_ij(i,:)))>0
            discount = kappa*(mean(delta_ij(i,:)))*vec; %% also add valence-dependent parameters: kappa for PE+, lambda for PE-
        else
